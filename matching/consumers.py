...
class ChatConsumer( AsyncWebsocketConsumer ):
    ...
    async def receive( self, text_data ):
        ...
        # チャットからの離脱時の処理
         ...
         # メッセージ受信時の処理
            ...
            # 音鳴らしてみたい
            pygame.mixer.music.load("./matching/static/audio/send.mp3") #読み込み
    ...
    async def join_chat( self, strRoomName ):
        ...
        # システムメッセージの作成
        strMessage = 'ユーザーID"' + self.strUserName + '"さんが参加しました。現在参加者は' + str( ChatConsumer.rooms[self.strGroupName]['participants_count'] ) + '人です。'
        # グループ内の全コンシューマーにメッセージ拡散送信（受信関数を'type'で指定）
        data = {
            'type': 'chat_message', # 受信処理関数名
            'message': strMessage, # メッセージ
        ...
        pygame.mixer.music.load("./matching/static/audio/join.mp3") #読み込み
        pygame.mixer.music.play(1) #再生
        await self.channel_layer.group_send( self.strGroupName, data )
    ...
    async def leave_chat( self ):
        ...
        ChatConsumer.rooms[self.strGroupName]['participants_count'] -= 1
        # システムメッセージの作成
        strMessage = 'ユーザーID"' + self.strUserName + '" さんが退出しました。現在参加者は ' + str( ChatConsumer.rooms[self.strGroupName]['participants_count'] )  + '人です。'
        # グループ内の全コンシューマーにメッセージ拡散送信（受信関数を'type'で指定）
        data = {
            'type': 'chat_message', # 受信処理関数名
            'message': strMessage, # メッセージ
